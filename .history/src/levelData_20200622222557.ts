...
export const getLevelData = memoizeOne(() => {
    ...
    const batch = (batchNumber:number)=>levels.filter(level=>Number(level[csvHeaders['batchNumber']]) === batchNumber)
    const level = (order:number)=>levels.filter(level=>Number(level[csvHeaders['order']]) === order)
    return {
        level:(order:number)=>batch(batchNumber).map(rowToLevel),
        levels:(batchNumber:number)=>batch(batchNumber).map(rowToLevel),
        batch,
        releasedBatches,
        newestBatch,
    } as levelData
