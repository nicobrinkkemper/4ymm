...
export default function Button({ children, primary, icon, href }: ButtonProps) {
    const Icon = createIcon(icon)
    const classList = ['Button', icon]
    if (primary) classList.push('primary')
    if (icons.hasOwnProperty(icon)) classList.push('hasIcon')
    const props = {
        href,
        className:classList.join(' ')
    }
    if(href.indexOf('http://') === 0 || href.indexOf('https://') === 0 || href.indexOf('//') === 0) Object.assign(props, {target:'_blank'}) 
    return (
        <a  {...props}>
            {children}
            <Icon />
