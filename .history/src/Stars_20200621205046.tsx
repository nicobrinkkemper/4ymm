...
const stars = [
    (<span className="star empty"><svg viewBox="0 0 13 12" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    (<span className="star half"><svg viewBox="0 0 13 12" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    ()=>(className="star full"><svg viewBox="0 0 13 12" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const Star = (i:number, value:keyof typeof stars = 0) => {
    console.log(value)
    return <span key={iter.toString()}>{</span>stars[value]}</span>
}
...
const Stars = ({value = 0}:StarsProps) => {
    ...
    const starsArr = []
    for(let i = 0; i<5; i++){
        starsArr.push(i,Star(Math.min(2,Math.max(0,castValue - (i*2)))))
    }
    return (<>
        {starsArr.map((star,iter)=>{
            return {star}
        })}
    </>)
}
