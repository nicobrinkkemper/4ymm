...
function Batch() {
  ...
  const batchLevels = levelData.levels(batchNumber).filter(({ batchNumber: _batchNumber }) => Number(_batchNumber) === Number(batchNumber))
        ...
        return <Card key={String(i)} to={`/level/${batchNumber}/${level.order}`} >
          <div className={"LevelCard"}>
            <picture>
              ...
              <img src={`./levelImages/${transformName(level.levelName)}-480.png`} style={{ height: '6.125rem', width: '6.125rem', objectFit: 'cover' }} />
            </picture>
            <div className="info">
