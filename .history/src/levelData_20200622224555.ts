...
export const getLevelData = memoizeOne(() => {
    ...
    const levels = (batchNumber:number)=>{
        if(typeof batchNumber !== 'number') throw(new Error('batchNumber should be number'))
        levelRows.filter(level=>{
            console.log({level, batchNumber}, csvHeaders['batchNumber'], Number(level[csvHeaders['batchNumber']]))
            return Number(level[csvHeaders['batchNumber']]) === castBatchNumber
        })
