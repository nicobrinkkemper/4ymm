...
function Batch() {
  ...
  const transformLevelName = (levelName: string) => snakeCase(levelName.toLowerCase())
  const levelPath = (levelName: string, width: number = 480) => `./levelImages/${transformLevelName(levelName)}-${width}.png`
        ...
        return <Card key={String(i)} to={`/level/${batchNumber}/${level.order}`} >
          <div className={"LevelCard"}>
            <picture>
              ...
              <img src={`${transformLevelName(level.levelName)}-480.png`} style={{ height: '6.125rem', width: '6.125rem', objectFit: 'cover' }} />
            </picture>
            <div className="info">
