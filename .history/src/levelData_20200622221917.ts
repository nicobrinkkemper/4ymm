...
export const getLevelData = memoizeOne(() => {
    ...
    return {
        levels:(batchNumber:number)=>levels.filter(level=>Number(level[csvHeaders['batchNumber']]) === batchNumber).map(rowToLevel),
        batches:(batchNumber:number)=>levels.filter(level=>Number(level[csvHeaders['batchNumber']]) === batchNumber).map(rowToLevel),
        releasedBatches,
        newestBatch,
    } as levelData
});