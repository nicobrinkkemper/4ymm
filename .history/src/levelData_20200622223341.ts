...
export const getLevelData = memoizeOne(() => {
    ...
    const level = (order:number)=>toLevel(levelRows.find(level=>Number(level[csvHeaders['order']]) === order) || [])
    const levels = (batchNumber:number)=>batch(batchNumber).map(toLevel)
    return {    
        level,
        levels,
        batch,
