...
const BackButton = () => {
  ...
  if(releaseDays[0].getTime() > Date.now()) return <Button icon="arrow-left-inverted" iconPosition="left" to={`/levels/${batchNumber}`} inverted={true}>Back to {new Intl.DateTimeFormat('en-US', { month: 'long', day: 'numeric' }).format(releaseDays[Number(batchNumber) - 1])}</Button>
...
function App() {
  ...
  const routes = (releaseDays[0].getTime() > Date.now()) ? (
    <Switch>
      {creditRoute}
      <Route path="/">
        <header className="App-header">
