...
const newestBatch = releaseDays.indexOf(releaseDays.sort((a,b)=>{
  return (a.getTime() - new Date().getTime()) - (b.getTime() - new Date().getTime())
})[0])
console.log(newestBatch)
  ...
  return (
    <div className="Levels">
      {releaseDays.map((releaseDay,i) => {
        const classList = [];
        if(newestBatch === i) classList.push('new')
        return (<Card key={String(i)} >
            ...
            {newestBatch === i ? <span className="new">New</span> : null}
            </div>
        </Card>
     )})}
    </div>
