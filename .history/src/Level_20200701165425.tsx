...
function Level() {
  const { batchNumber:strBatchNumber, order } = useParams();
  const levelData = getLevelData()
  const batchLevels = levelData.batch(Number(strBatchNumber))
  const level = levelData.level(Number(order))
  ...
  const isUnreleased = levelData.releasedBatches.indexOf(releaseDay) === -1 || Number(strBatchNumber) !== level.batchNumber
