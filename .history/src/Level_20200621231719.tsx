...
function Level() {
  ...
  const hasNextLevel = batchLevels.length > order;
  const hasPreviousLevel = order>1;
  ...
  return (
    <div className="Level">
      ...
      {hasPreviousLevel ? <Button icon="arrow-left" iconPosition="left" to={`/levels/${Number(batchNumber)}/${Number(order)-1}`}>Previous level</Button> : null}
      {hasNextLevel ? <Button icon="arrow-right" to={`/levels/${Number(batchNumber)}/${Number(order)+1}`}>Next level</Button> : null}
    </div>
  )
}
