...
function Level() {
  ...
  return (
    <div className="Level">
      ...
      {hasPreviousLevel ? <Button icon="arrow-left" iconPosition="left" to={`/levels/${batchNumber}/${Number(order)-1}`} /> : null}
      {hasNextLevel ? <Button icon="arrow-right" to={`/levels/${batchNumber}/${Number(order)+1}`} /> : null}
    </div>
  )
}
